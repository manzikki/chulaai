...
def print_ocean_no_fleet(ocean):
    header = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']
    print("   " + " ".join(header))  # Print column labels
    for i, row in enumerate(ocean, start=1):
        row_str = " ".join(row)
        row_str = row_str.replace('a',' ')
        row_str = row_str.replace('b',' ')
        row_str = row_str.replace('c',' ')
        row_str = row_str.replace('s',' ')
        row_str = row_str.replace('d',' ')
...
while any(ship[0] in row for row in ocean1 for ship in ships) and any(ship[0] in row for row in ocean2 for ship in ships):
        if current_player == 1:
            print("Player 2's ocean without the fleet")
            print_ocean_no_fleet(ocean2) #change this!
